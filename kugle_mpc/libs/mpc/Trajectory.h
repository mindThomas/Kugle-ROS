...
namespace MPC
{
	...
	class Trajectory
	{
		public:
            ...
            void AddPoint(unsigned int seq, double x, double y, double heading = -1, double velocity = -1);
            void clear();
			size_t size();
            ...
            void SequenceExtract(Trajectory& extractedTrajectory, unsigned int seq_min, unsigned int seq_max = std::numeric_limits<unsigned int>::infinity());
            ...
            void print();
            bool find(TrajectoryPoint& foundPoint, unsigned int seq);
            void sort();
            double distance();
