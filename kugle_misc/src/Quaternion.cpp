...
#define arm_scale_f32(in, scale, out, len) { int __i__; for (__i__ = 0; __i__ < len; __i__++) out[__i__] = scale * in[__i__]; }
...
/* mat = Phi(q) */
void Quaternion_mat_Phi(const double q[4], double mat[4*4])
{
	/*Phi^T = @(q)[q(0) q(1) q(2) q(3);     % for q o p = Phi(q) * p
				  -q(1) q(0)  q(3) -q(2);
				  -q(2) -q(3)  q(0)  q(1);
...
/* mat = Phi(q) * Λ  */
void Quaternion_mat_PhiVec(const double q[4], double mat[4*3])
{
	/*Phi^T = @(q)[q(0) q(1) q(2) q(3);     % for q o p = Phi(q) * p
				  -q(1) q(0)  q(3) -q(2);
				  -q(2) -q(3)  q(0)  q(1);
...
void Quaternion_devecGammaT(const double p[4], const double q[4], double result[3])
{
	/*Gamma^T = @(p)[p(0) p(1) p(2) p(3);   % for q o p = Gamma(p) * q
					-p(1) p(0) -p(3) p(2);
					-p(2) p(3) p(0) -p(1);
...
/* mat = Gamma(p) */
void Quaternion_mat_Gamma(const double p[4], double mat[4*4])
{
	/*Gamma = @(p)[p(0) -p(1) -p(2) -p(3);   % for q o p = Gamma(p) * q
				   p(1) p(0) p(3) -p(2);
				   p(2) -p(3) p(0) p(1);
...
/* q = -q */
void Quaternion_Negate(double q[4])
{
	q[0] = -q[0];
	q[1] = -q[1];
	q[2] = -q[2];
...
void Quaternion_RotateVector_Body2Inertial(const double q[4], const double v[3], double v_out[3])
{
...
void Quaternion_RotateVector_Inertial2Body(const double q[4], const double v[3], double v_out[3])
{
...
void Quaternion_GetAngularVelocity_Inertial(const double q[4], const double dq[4], double omega_inertial_out[3])
{
...
void Quaternion_GetAngularVelocity_Body(const double q[4], const double dq[4], double omega_body_out[3])
{
...
void Quaternion_GetDQ_FromInertial(const double q[4], const double omega_inertial[3], double dq[4])
{
	// dq = 1/2 * q_omega_body o q
	// dq = 1/2 * Gamma(q) * [0;q_omega_inertial];
	double omega_q[4] = {0, omega_inertial[0], omega_inertial[1], omega_inertial[2]};
	Quaternion_Phi(q, omega_q, dq); // Gamma(q) * [0;omega_inertial]
	arm_scale_f32(dq, 0.5f, dq, 4);
}

void Quaternion_GetDQ_FromBody(const double q[4], const double omega_body[3], double dq[4])
{
	// dq = 1/2 * q o q_omega_body
	// dq = 1/2 * Phi(q) * [0;q_omega_body];
	double omega_q[4] = {0, omega_body[0], omega_body[1], omega_body[2]};
...
double HeadingFromQuaternion(const double q[4])
{
	...
	double heading = atan2(I_e_x[1], I_e_x[0]);

	return heading;
}

void HeadingQuaternion(const double q[4], double q_heading[4])
{
